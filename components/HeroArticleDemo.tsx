...
function AvatarStack({ names, size = 18, max = 2 }) {
...
function KanbanColumn({ colKey, tasks }) {
  function KanbanColumn({ colKey, tasks }: { colKey: keyof typeof COL_META; tasks: (typeof PHASE1_TASKS)[] }) {
...
export default function CrabsHQDemo() {
  ...
  const [tasks, setTasks] = useState<(typeof PHASE1_TASKS)[]>([]);
  ...
  const processStep = useCallback((idx) => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      if (s.type === "typing") {
        ...
        typeRef.current = setInterval(() => { ci++; setInputText((s.text || "").slice(0, ci)); if (ci >= (s.text || "").length) { clearInterval(typeRef.current!); typeRef.current = null; setScriptIndex(idx + 1); } }, 28);
        return;
      }
      ...
      if (s.type === "nick_typing") { setNickTyping(true); setActiveAgents(p => new Set([...p, "Nick"])); setScriptIndex(idx + 1); return; }
      if (s.type === "response") { setNickTyping(false); setActiveAgents(p => new Set([...p, s.sender])); setMessages(p => [...p, { sender: s.sender, role: s.role, text: s.text, isHuman: false, time: s.time }]); setScriptIndex(idx + 1); return; }
      if (s.type === "reaction") { setMessages(p => { const c = [...p]; if (c.length) c[c.length - 1] = { ...c[c.length - 1], reaction: { emoji: s.emoji, count: s.count } }; return c; }); setScriptIndex(idx + 1); return; }
      if (s.type === "addTasks") { setTasks(p => [...p, ...(s.phase === 1 ? PHASE1_TASKS : PHASE2_TASKS)]); setScriptIndex(idx + 1); return; }
    }, s.delay);
  }, []);
  ...
  const cols: { [key in keyof typeof COL_META]: (typeof PHASE1_TASKS)[] } = { inbox: [], assigned: [], progress: [], review: [] };
  tasks.forEach((t: (typeof PHASE1_TASKS)[number]) => { if (cols[t.col as keyof typeof COL_META]) cols[t.col as keyof typeof COL_META].push(t); });
