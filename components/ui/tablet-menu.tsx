...
export default function TabletMenu() {
  ...
  useEffect(() => {
    if (!tabletNavOpen) return;
    
    const clickHandler = ({ target }: { target: EventTarget | null }): void => {
      if (!tabletNav.current || !trigger.current) return;
      if (tabletNav.current.contains(target as Node) || trigger.current.contains(target as Node)) return;
      ...
      >
        ...
        >
          <ul className="px-5 py-4 space-y-4">
            <li>
              <Link href="https://trooper.lemonsqueezy.com/affiliates" className="font-medium text-slate-600 hover:text-slate-900 px-5 py-3 flex items-center transition duration-150 ease-in-out w-full justify-between border-b-2" onClick={() => {
                setTabletNavOpen(false);
                setDropdownOpen(false);
              }}>
                Affiliate
            ...
            {/* Pricing */}
            <li>
              <Link href="/pricing" className="font-medium text-slate-600 hover:text-slate-900 px-5 py-3 flex items-center transition duration-150 ease-in-out w-full justify-between border-b-2" onClick={() => {
                setTabletNavOpen(false);
                setDropdownOpen(false);
              }}>
                Pricing
                  ...
                  <ul className="space-y-2 px-2">
                    <li className="flex items-start p-2 sm:p-3 transition duration-150 ease-in-out rounded-lg hover:bg-slate-100">
                      <Link href="/create-a-helpdesk-servicedesk-notion" className="flex items-start w-full" onClick={() => {
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="w-5 h-5 sm:w-6 sm:h-6 text-emerald-600 flex-shrink-0 mt-0.5">
                    ...
                    <li className="flex items-start p-2 sm:p-3 transition duration-150 ease-in-out rounded-lg hover:bg-slate-100">
                      <Link href="/create-a-documentation-notion" className="flex items-start w-full" onClick={() => {
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-5 h-5 sm:w-6 sm:h-6 text-emerald-600 flex-shrink-0 mt-0.5">
            ...
            <li className="pt-2">
              ...
              >
                ...
                <div className="absolute bg-emerald-500 w-0 h-full left-0 top-0 group-hover:w-full transition-all duration-300 ease-in-out -z-0"></div>
