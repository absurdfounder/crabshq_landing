...
function GmailPane() {
  const rows = [
    ...
    {
      from: 'GitHub',
      sub: '[wonderdesk] Deploy succeeded',
      snip: ' — main @ 8f2a1c9',
      time: 'Sun',
      unread: false,
      domain: 'github.com',
    ...
    {
      from: 'Figma',
      sub: 'Board pack cover — comments',
      snip: ' — 3 new comments from Ren',
      time: 'Sat',
      unread: false,
      domain: 'figma.com',
    },
    ...
    <div className="flex h-full min-h-0 bg-white text-[11px] text-[#202124]">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          {rows.map((r) => {
            const hotspot = r.sub.includes('Q3 ledger');
            return (
            <div
              key={r.sub}
              {...(hotspot
                ? { 'data-demo-target': 'browser-hotspot', 'data-claim-tab': 'gmail' }
                : {})}
              ...
              } ${hotspot ? 'ring-2 ring-inset ring-[#1a73e8]/40' : ''}`}
            >
...
function StripePane() {
  const rows = [
    ...
    { amt: '$640.00', status: 'Succeeded', desc: 'Harbor · Seat expansion', date: 'Jul 25', method: 'Visa ···· 4242' },
    { amt: '$1,150.00', status: 'Succeeded', desc: 'Pixel Forge · Monthly', date: 'Jul 24', method: 'Mastercard ···· 4444' },
  ];
        ...
        <div className="flex flex-wrap items-center gap-1.5 border-b border-[#e3e8ee] bg-white px-3 py-1.5">
          {['Succeeded', 'Last 30 days', 'USD'].map((chip) => (
            <span
              key={chip}
              {...(chip === 'Succeeded'
                ? { 'data-demo-target': 'browser-hotspot', 'data-claim-tab': 'stripe' }
                : {})}
              className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-medium ${
                chip === 'Succeeded'
                  ? 'border-[#635bff] bg-[#f0efff] text-[#635bff] ring-2 ring-[#635bff]/25'
  ...
  return (
    <div className="flex h-full min-h-0 bg-[#f4f5f8] text-[11px] text-[#393a3d]">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          {rows.map((r) => {
            const hotspot = r.num === '1000';
            return (
            <div
              key={r.num}
              {...(hotspot
                ? { 'data-demo-target': 'browser-hotspot', 'data-claim-tab': 'qbo' }
                : {})}
              className={`grid grid-cols-[44px_1.4fr_0.75fr_0.9fr] gap-2 border-b border-[#e5e7eb] bg-white px-3 py-2 ${
                hotspot ? 'bg-[#e8f5e9] ring-2 ring-inset ring-[#2ca01c]/35' : ''
              }`}
            >
...
function NotionPane() {
  ...
  return (
    <div className="flex h-full min-h-0 bg-white text-[11px] text-[#37352f]">
      ...
      <div className="min-w-0 flex-1 overflow-y-auto px-3 py-2.5 sm:px-4">
        ...
        <div className="mt-2.5 space-y-1.5 border-t border-black/[0.06] pt-2.5">
          {todos.map((t) => {
            const hotspot = t.text.includes('Reconcile');
            return (
            <div
              key={t.text}
              {...(hotspot
                ? { 'data-demo-target': 'browser-hotspot', 'data-claim-tab': 'notion' }
                : {})}
              className={`flex items-start gap-2 rounded-md px-1 py-0.5 ${
                hotspot ? 'bg-[#e8f0fe] ring-2 ring-inset ring-[#2383e2]/30' : ''
              }`}
            >
              <span
